...
class DBView:
    ...
    # new stats fetcher with custom date picking
    def new_stats(self, start, end):
        with self.conn:
                self.c.execute(
                    '''
                    SELECT name, count(telegram_bttns.telegram_id), timestamp 
                    FROM telegram_bttns 
                    INNER JOIN telegram_users ON telegram_users.telegram_id = telegram_bttns.telegram_id
                    WHERE timestamp 
                    BETWEEN :start AND :end
                    GROUP BY telegram_bttns.telegram_id 
                    ORDER BY count(telegram_bttns.telegram_id) 
                    DESC
                    ''',
                    {
                        'start': start,
